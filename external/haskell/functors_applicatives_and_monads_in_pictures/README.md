...
# Functors
 ...
 어떠한 상태에 놓여진 값에 대해, 평범한 함수를 적용할 수는 없습니다.
...
```
 ...
 `fmap`은 길거리 출신이며, 상태에 대해서 빠삭합니다. `fmap`은 상태로 감싸진 값에 어떻게 함수를 적용해야 할지를 알고 있습니다. 예를 들어, `Just 2`에 `(+3)`를 적용시켜 봅시다. `fmap`을 이용해보면, 
...
![functor_def.png]
 ...
 `Functor`는 fmap을 적용시키는 방법을 정의한 데이터 타입입니다. 여기, 어떻게 fmap이 동작하는지 나와 있습니다.
...
# Applicatives
 ...
 다음 단계로 넘어가보도록 하겠습니다. Functors에서 나왔던 것처럼, 상태에 값을 넣습니다.

![value_and_context.png]

 함수 역시 어떠한 상태에 놓여질 수 있습니다.

![function_and_context.png]


 그럼 자세히 살펴보도록 하겠습니다. Applicatives는 바보가 아닙니다. Control.Applicative는 상태로 감싸진 값에, 상태로 감싸진 함수를 적용하는 법을 알도록 `<*>`를 정의했습니다.
...
![applicative_list.png]
 ...
 여기, Functor로는 할 수 없지만, Applicative로 할 수 있는게 나와있습니다. 어떻게 두개의 인자를 취하는 함수를, 두개의 상태로 감싸진 값에 적용시킬까요?
...
```
 ...
 "`<$>`와 `<*>`를 가지고, 감싸여 있는 값들을, 감싸여 있지 않은 값들을 처리할 수 있는 함수에 넘겨주면, 싸여진 값을 얻을 수 있다고! 하하하하!"
...
2. 필요없으니 집어 치운다!
 ...
 Functors는 감싸여 있는 값에 함수를 적용할 수 있습니다.

![fmap.png]

Applicatives는 감싸여 있는 값에, 감싸여 있는 함수를 적용 할 수 있습니다.

![applicative.png]

 Monads는 감싸여진 값에 함수를 적용시켜 __감싸여진 값을 반환__할 수 있습니다.
...
![half_ouch.png]
 ...
 함수에 감싸진 값을 밀어 넣을려면, `>>=`가 필요합니다. 여기, `>>=`의 사진이 있습니다.
...
```
 ...
 여기 나온 세개의 함수는 평범한 값(혹은 없거나)을 받아, 감싸진 값을 반환합니다. 따라서, 우리는 이를 `>>=`로 묶을 수 있습니다!
...
* functors: `fmap`이나 `<$>`를 이용하여 감싸진 값에 함수를 적용할 수 있다.
* applicatives: `liftA`나 `<*>`를 이용하여 감싸진 함수를 감싸진 값에 적용할 수 있다.
* monads: `liftM`나 `>>=`를 이용하여 감싸진 값을 받아 함수를 적용하고 감싸진 값을 반환할 수 있다.
